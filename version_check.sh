...
chart_app_version=$(yq eval .appVersion kubernetes/cray-dns-powerdns/Chart.yaml)
chart_image_version=$(cat kubernetes/cray-dns-powerdns/Chart.yaml |grep -m 1 image:|cut -d ':' -f3)
chart_values_global_chart_version=$(yq eval .global.chart.version kubernetes/cray-dns-powerdns/values.yaml)
chart_values_global_app_version=$(yq eval .global.appVersion kubernetes/cray-dns-powerdns/values.yaml)
